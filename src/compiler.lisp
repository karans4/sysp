...
(defun compile-program (forms &optional (out t))
  ...
  (reset-program-state)
  (let (includes structs extern-structs defines externs defns trait-defns)
    (dolist (f forms)
      (case (first f)
        ...
        (enum          (dolist (variant (cddr f))
                         (push (list 'define (first variant) (second variant))
                               defines)))
        (defn          (push f defns))
        (deftrait      (register-deftrait f))
        (impl          (setf trait-defns
                             (nconc trait-defns (register-impl f))))
    ...
    (setf includes        (nreverse includes)
          ...
          defns           (nconc (nreverse defns) trait-defns))
